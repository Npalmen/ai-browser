...
V7  persistent workflows — COMPLETE
    durable definitions, structured scheduling, restart-safe queue/history
    fresh V6 live execution, per-action V4 approval, Workflows product surface
    no closed-app execution
    ADR-008

V8  AI-native browser — COMPLETE
    trusted omnibox Navigate/Search/Ask/Act/Delegate/Automate
    explicit multi-tab read-only context
    WorkflowDraft → trusted V7 Save
    Activity/attention projection
    ADR-009
