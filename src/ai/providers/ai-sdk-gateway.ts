...
const WORKFLOW_DRAFT_SCHEMA = jsonSchema<WorkflowDraft>(
  WORKFLOW_DRAFT_JSON_SCHEMA as unknown as JSONSchema7,
  {
    validate: (value: unknown) => {
      try {
        const parsed = parseWorkflowDraft(value);
        return { success: true as const, value: parsed };
      } catch (error: unknown) {
        const message = error instanceof Error ? error.message : 'Invalid workflow draft.';
        return { success: false as const, error: new Error(message) };
      }
    },
  },
...
export interface GatewayStreamTextArgs {
  ...
  outputSchema?: 'pageAnswer' | 'agentModelOutput' | 'autonomousTaskDecision' | 'workflowDraft';
}
...
function defaultStreamText(args: GatewayStreamTextArgs): GatewayStreamTextResult {
  ...
  if (args.outputSchema === 'workflowDraft') {
    return defaultWorkflowDraftStreamText(args);
  }
...
function defaultWorkflowDraftStreamText(args: GatewayStreamTextArgs): GatewayStreamTextResult {
  const result = streamText({
    model: args.model,
    messages: args.messages,
    maxOutputTokens: args.maxOutputTokens,
  ...
  async generateWorkflowDraft(
    ...
    try {
      ...
      const result = this.streamText({
        ...
        abortSignal,
        providerOptions,
        outputSchema: 'workflowDraft',
      });

      const draft = asWorkflowDraft(await result.output);
      const usage = normalizeModelUsage(await result.usage);
      const cost = normalizeGatewayCost(await result.providerMetadata);
      const resolvedProviderModelId = resolveProviderModelId(
        await result.response,
...
function asWorkflowDraft(value: unknown): WorkflowDraft {
  try {
    return parseWorkflowDraft(value);
  } catch (error) {
