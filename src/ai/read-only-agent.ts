...
export interface ReadOnlyAgentDependencies {
...
export class ReadOnlyAgent {
  ...
  constructor(dependencies: ReadOnlyAgentDependencies) {
  ...
  async answer(request: AgentRequest, options: AgentAnswerOptions = {}): Promise<AgentAnswer> {
    if (request.abortSignal?.aborted) {
      throw cancelledError();
    }

    const question = normalizeUserQuestion(request.question);
  ...
  private async executeAsk(input: {
    ...
    const builtContext = buildModelPageContext(observation);
  ...
  private prepareModelCall(input: {
    ...
    builtContext: BuiltModelPageContext;
