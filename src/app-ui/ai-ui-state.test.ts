...
describe('AI UI event ordering', () => {
  it('Case A: acknowledgement then started/text/finished yields one completed assistant', () => {
    const createId = ids();
    let state: AiUiState = {};
    state = appendUserQuestion(state, TAB, 'What is this?', createId);
    state = acknowledgeAsk(state, TAB, ASK_A, createId);
  ...
  it('Case G: late events from ask A do not alter ask B', () => {
    const createId = ids();
    let state: AiUiState = {};
    state = appendUserQuestion(state, TAB, 'Question A?', createId);
    state = applyAiAnswerEvent(state, started(ASK_A), createId);
    state = applyAiAnswerEvent(state, delta(ASK_A, 'From A'), createId);
    state = appendUserQuestion(state, TAB, 'Question B?', createId);
    state = applyAiAnswerEvent(state, started(ASK_B), createId);
    state = acknowledgeAsk(state, TAB, ASK_B, createId);
  ...
  it('keeps tab transcripts isolated', () => {
    const createId = ids();
    let state: AiUiState = {};
    state = appendUserQuestion(state, 'tab-1', 'One?', createId);
    ...
    state = appendUserQuestion(state, 'tab-2', 'Two?', createId);
